...
addpath(genpath([main_dir filesep 'code' filesep 'eeg' filesep 'preprocessing' filesep 'MADE-EEG-preprocessing-pipeline']));% enter the path of the folder in this line
...
rawdata_location = [main_dir filesep 'sourcedata' filesep 'raw' filesep 's1_r1' filesep 'eeg'];

% 2. Enter the path of the folder where you want to save the processed data
output_location = [main_dir filesep 'derivatives' filesep 'preprocessed' filesep 'eeg' filesep 'preprocessed' ];

% 3. Enter the path of the channel location file
channel_locations = [main_dir filesep 'code' filesep 'eeg' filesep 'preprocessing' filesep 'MADE-EEG-preprocessing-pipeline' filesep 'chan_locs_files' filesep 'CACS-128-X7-FIXED-64only.bvef'];

% stimulus markers during practice are S13, S14, S15, and S16 
% and markers during the main flanker task of mfe_jr are S5, S6, S7, and S8!
stimulus_markers = {'S  5', 'S  6', 'S  7', 'S  8'};      % enter the stimulus makers that need to be adjusted for time offset
response_markers = {'S  11', 'S  12', 'S  21', 'S  22'};       % enter the response makers that need to be adjusted for time offset

% 4. Does your data need correction for stim/response time offset?
adjust_time_offset = 1; % 0 = NO (no correction), 1 = YES (correct time offset)
% If your data need correction for time offset, specify the offset time (in milliseconds)
stimulus_timeoffset   = 10.56; % stimulus related time offset (in milliseconds). 0 = No time offset
...
task_event_markers = {'S  5', 'S  6', 'S  7', 'S  8', 'S 11', 'S 12', 'S 21', 'S 22'}; % enter all the event/condition markers (i.e., stim + resp markers)
...
parent_folder = rawdata_location;

% Validate parent folder
if ~isfolder(parent_folder)
    error('Parent folder "%s" does not exist. Please provide a valid path.', parent_folder);
end

% Find all .vhdr files recursively and get their info
file_info_struct = dir(fullfile(parent_folder, '**', '*.vhdr'));

% Filter out any directories (safeguard, as '*.vhdr' should primarily match files)
file_info_struct = file_info_struct(~[file_info_struct.isdir]);

% Create a cell array of full file paths
if ~isempty(file_info_struct)
    % Construct full paths in a vectorized way from the struct fields
    datafile_names = fullfile({file_info_struct.folder}, {file_info_struct.name});
    % Ensure 'datafile_names' is a column cell vector for consistency
    datafile_names = datafile_names(:);
else
    datafile_names = {}; % Return an empty cell array if no files are found
end

% The line below extracts path, name, and extension from the *first* found file.
% - 'filepath_of_first' will be the directory path of the first file.
% - 'name_of_first' will be the filename (without extension) of the first file.
% - 'ext' will be the file extension of the first file (e.g., '.vhdr').
% This 'ext' variable is used later in the script (e.g., in strrep functions).
% This line will work correctly even if datafile_names{1} contains a full path.
[filepath_of_first, name_of_first, ext] = fileparts(datafile_names{1});
...
%for subject = 1:numel(datafile_names)
for subject = 7
    ...
    current_full_path = datafile_names{subject};
    % Extract the filename without path and without extension
    % fileparts returns: [filepath, name, ext]
    [~, filename_to_display, ~] = fileparts(current_full_path);

    fprintf('\n\n\n*** Processing subject %d of %d (%s) ***\n\n\n', subject, numel(datafile_names), filename_to_display);

    datafile_names{subject} = extractAfter(datafile_names{subject}, rawdata_location); % Kia had to add this due to changes I made above
    ...
    % Kia: datafile_names{subject} should be changed again such that it no longer
    % contains the /sub-37000xx/ part to avoid errors after this line of
    % code. 
    datafile_names{subject} = extractSubjectID(datafile_names{subject});
    ...
    newChanlocs = loadbvef(channel_locations);
...
%     EEG = pop_loadset( 'filename', strrep(datafile_names{subject}, ext, '_processed_data_immediate.set'), 'filepath', [output_location filesep 'processed_data' filesep]);
    ...
    % As WriteMode function was not available in MATLAB R2019b (HPC does
    % not have newer versions of MATLAB), I have to change the code below
    % and comment the existing one.
